...
RSpec.describe(Merchant, type: :model) do
  ...
  describe("instance methods") do
    ...
    describe 'discount' do
      ...
      let!(:bulk_discount_2) {merchant_2.bulk_discounts.create!(percentage_discount: 30, quantity_threshold: 15)}
      let!(:transaction_1) {Transaction.create!(invoice: invoice_1, credit_card_number: 4654405418249632, credit_card_expiration_date: '', result: 'success')}
      let!(:transaction_2) {Transaction.create!(invoice: invoice_2, credit_card_number: 4654405418233632, credit_card_expiration_date: '', result: 'success')}

      let!(:item_1) {merchant_1.items.create!(name: "10 lb bag of flour", description: "10 pounds of it", unit_price: 1000)}
      let!(:item_1) {merchant_2.items.create!(name: "Hair pins", description: "10 per pack", unit_price: 500)}

      let!(:invoice_item_1) {InvoiceItem.create!(item: item_1, invoice: invoice_1, unit_price: 1000, quantity: 10, status: 2)}
      let!(:invoice_item_2) {InvoiceItem.create!(item: item_2, invoice: invoice_1, unit_price: 500, quantity: 15, status: 2)}

      it 'calculates the total discount for each merchant from an invoice' do
        expect(merchant_1.discount).to eq(200)
        expect(merchant_2.dicsount).to eq(150)
      end

      xit 'returns zero if a merchant has no invoices' do
        expect(merchant_3.discount).to eq(0)
      end
    end
